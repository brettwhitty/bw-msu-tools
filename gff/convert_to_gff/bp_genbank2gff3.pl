...
#$Id: genbank2gff3.PLS,v 1.16 2007/10/01 15:18:13 scain Exp $;
...
## for preventing duplicate IDs in GFF
my %gene_id_counter = ();
...
#        print  Dumper $seq;
#        print total_size($in)."B\n";
#        print total_size($unflattener)."B\n";
#       print total_size($tm)."B\n";
#        print total_size($idh)."B\n";
        ...
        if ( $lump ) {
            ...
            open $out, ">$outfile";
            $outfa = $outdir . $seq_name . ".fa";  # this is a temp file ... see below
            open $lumpfa_fh, ">$outfa" or die "Could not create a lump outfile called ($outfa) because ($!)\n";
        }
...
#    $source_type= "chromosome"; 
    ...
    if ($chrname =~ /^\d+$/) {
        $sf->add_tag_value( Alias => "Chr$chrname" );
    } else {
        $sf->add_tag_value( Alias => $chrname );
    }
  }

  ## add the clone name as an alias
  if ($sf->has_tag('clone')) {
    my ($clonename) = $sf->get_tag_values('clone');
    $sf->add_tag_value( Alias => $clonename );
  }

  ## add Dbxrefs for GenBank accessions
  my $gi_xref = 'NCBI_gi:'.$sf->entire_seq->primary_id;
  my $gb_xref = 'NCBI_gb:'.$sf->entire_seq->display_id;

  ## add the sequence version number (if it has one)
  if ($sf->entire_seq->version()) {
    my ($version) = $sf->entire_seq->version();
    $sf->add_tag_value( version => $version );
    $gb_xref .= '.'.$version;
  }
  ...
  $sf->add_tag_value( comment1 => $comment->text() ) if(!$is_swiss && $comment && ! $sf->has_tag('comment1'));
...
#   $sf->add_tag_value( Dbxref => $_ ) foreach(@dblink);
  ...
  ## add the sequence ID as Name if we don't have a name assigned yet (for gbrowse) 
  if (! $sf->has_tag('Name')) {
    $sf->add_tag_value( Name => $seq->id() );
  }
...
#    $gene_id_counter{$gene_id}++;
#
#    if ($gene_id_counter{$gene_id} > 1) {
#        $gene_id .= '-' . $gene_id_counter{$gene_id};
#    }
