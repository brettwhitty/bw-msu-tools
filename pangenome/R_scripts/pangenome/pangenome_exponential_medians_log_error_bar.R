...
start=list(th1=15000, th2=15000, th3=15))
#summary(nlmodel_exp)
...
# Draw the axis
plot(V1,V2, ylim=c(min(V2),(nlmodel_exp$m$getPars()[1]*1.01)), xlab="number of genomes", ylab="number of genes", main="###TITLE### pangenome exponential log axis", cex=0.5, log="xy", type="n")
...
expr_exp <- substitute(
                expression(y == th1err %+-% th1 + th2err %+-% th2 * italic(e)^(-x / (th3 %+-% th3err))), 
                list(
                    th1 = round(nlmodel_exp$m$getPars()[1], digit=2),
                    th1err = round(summary(nlmodel_exp)[10][[1]][4], digit=2),
                    th2 = round(nlmodel_exp$m$getPars()[2], digit=2),
                    th2err = round(summary(nlmodel_exp)[10][[1]][5], digit=2),
                    th3 = round(nlmodel_exp$m$getPars()[3], digit=2),
                    th3err = round(summary(nlmodel_exp)[10][[1]][6], digit=2)
                    )
                )
