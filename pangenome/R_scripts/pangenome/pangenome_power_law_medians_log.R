...
start=list(th1=-10000, th2=-0.5))
#summary(nlmodel_pot)

# Open up the output file for the log graph
postscript(file="###output_path###pangenome_power_law_medians_log.ps", width=11, height=8.5, paper='special')
layout(matrix(c(1,2),byrow=TRUE), heights=c(7.5,1))

# Draw the axis
plot(V1,V2, ylim=c(min(V2),max(V2)), xlab="number of genomes", ylab="number of genes", main="###TITLE### pan-genome power law log axis", cex=0.5, log="xy")
