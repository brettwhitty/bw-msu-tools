#!/opt/rocks/bin/perl
# $Id: converter.pl,v 1.4 2005/11/17 14:23:29 hunter Exp $
...
# MAIN
########################################################
my ($format, $input, $output, $jobid, $pfile, $res, $msg, $help, $region);
my $params = { };
...
########################################################
    ...
    my $infh;
    if ($input =~ /\.gz$/) {
        open $infh, '<:gzip', $input or usage("Failed opening input file '$input': $!\n");
    } else {
        open($infh, "< $input") || usage("can't open input file $input : $!\n");
    }
    while (<$infh>) 
    {
...
;
            ...
            if ($k =~ /^(IPR|NULL)/) {
                ...
                if ($results{$seq}{$k}{'name'} && $results{$seq}{$k}{'name'} ne 'NULL') {
                    ...
                    $name = encode($name);
                    $SetFeatNote = ";Note=$name";
                    if ($results{$seq}{$k}{'class'}) {
                        my $go = $results{$seq}{$k}{'class'};
                        $go = encode($go);
                        #$go =~ s/([^A-Za-z0-9])/sprintf("%%%02X", ord($1))/seg;
                        $SetFeatNote .= ",$go";
                    }
                }
                ...
                foreach my $mm (sort keys %{$results{$seq}{$k}{'meth'}}) {
                    ...
                    foreach my $hit (sort keys %{$results{$seq}{$k}{'meth'}{$mm}}) {
                        ...
                        my @dbxref = ();
                        if ($k ne 'NULL') {
                            push (@dbxref, "InterPro:$k");
                        }
                        if ($mm && $mm !~ /^Seg$/i && $mm !~ /^Signalp$/ && $mm !~ /^Tmhmm$/) {
                            my $lc_m = $mm;
                            $lc_m =~ tr/A-Z/a-z/;
                            my $dbxref_acc = $hit;
                            $dbxref_acc = encode($dbxref_acc);
                            #$dbxref_acc =~ s/([^A-Za-z0-9])/sprintf("%%%02X", ord($1))/seg;
                            push (@dbxref, $databases->{$lc_m}.":".$dbxref_acc);
                        }
                        ...
                        $SetFeatName = encode($SetFeatName);
                        #$SetFeatName =~ s/([^A-Za-z0-9])/sprintf("%%%02X", ord($1))/seg;

                        my $SetFeatAlias = $hit;
                        $SetFeatAlias = encode($SetFeatAlias);
                        #$SetFeatAlias =~ s/([^A-Za-z0-9])/sprintf("%%%02X", ord($1))/seg;
                        
                        $SetFeat  = "ID=$SetFeatID;Name=$SetFeatName;Alias=$SetFeatAlias";
                        $SetFeat .= ";Tool=$mm";
                        $SetFeat .= $SetFeatNote;
                        ## add dbxref
                        if (@dbxref) {
                            $SetFeat .= ";Dbxref=".join(",", @dbxref);
                        }
...
#                            my $PartFeatName = $mm . "_" . $results{$seq}{$k}{'meth'}{$mm}{$hit}{'name'};
                            my $PartFeatName = $results{$seq}{$k}{'meth'}{$mm}{$hit}{'name'};
                            
                            $PartFeatName = $PartFeatName =~ /no description/i ? $mm . "_" . $hit : $PartFeatName;
                            
                            $PartFeatName = encode($PartFeatName);
                            #$PartFeatName =~ s/([^A-Za-z0-9])/sprintf("%%%02X", ord($1))/seg;
                        
                            my $PartFeatAlias = $hit;
                            $PartFeatAlias = encode($PartFeatAlias);
                            #$PartFeatAlias =~ s/([^A-Za-z0-9])/sprintf("%%%02X", ord($1))/seg;
                            
                            $PartFeat  = "ID=$PartFeatID;Name=$PartFeatName;Alias=$PartFeatAlias;Parent=$SetFeatID";
                            $PartFeat .= ";Tool=$mm";
                            $PartFeat .= $SetFeatNote;
                            ## add dbxref
                            if (@dbxref) {
                                $PartFeat .= ";Dbxref=".join(",", @dbxref);
                            }
