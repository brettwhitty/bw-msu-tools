...
java -jar /run/whitty/treebuilder_pipeline/bin/readseq.jar -inform=12 -f=8 -o ${DIRNAME}/${PREFIX}.gfsa ${DIRNAME}/${PREFIX}.phyi

## run Gblocks to trim MSA
echo "## running Gblocks" 1>&2
/run/whitty/treebuilder_pipeline/bin/Gblocks ${DIRNAME}/${PREFIX}.gfsa -t=p -e=.trim -p=t -b5=h

## convert Gblocks output to input for phylip
echo "## converting Gblocks output to phylip format" 1>&2
java -jar /run/whitty/treebuilder_pipeline/bin/readseq.jar -inform=8 -f=12 -o ${DIRNAME}/${PREFIX}.phyi.trim ${DIRNAME}/${PREFIX}.gfsa.trim
